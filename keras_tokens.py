...
tar = tarfile.open('data/babi_tasks.tar.gz') #path
...
print('Length:', len(test_stories))
print('Examples:', test_stories[35])
'''
At this point I have a list of tuples
In each tuple, there are 3 things
  1. list of input tokens    (story)
  2. list of query tokens    (q)
  3. answer string           (answer)

vocab = sorted(reduce(lambda x, y: x | y, (set(story + q + [answer]) for story, q, answer in train_stories + test_stories)))
vocab = sorted(reduce(lambda x, y: x | y, (set(story + q + [answer]) for story, q, answer in "all stories")))
vocab = sorted(reduce(lambda x, y: x | y, (set("all unique words") for story, q, answer in "all stories")))
vocab = sorted(reduce(lambda x, y: x | y, (set of "all unique words") "per tuple"))
vocab = sorted(reduce(lambda x, y: x | y, "list of (sets of all unique words) per tuple"))
vocab = sorted(reduce("function that joins sets together", "list of sets of all unique words per tuple"))
vocab = sorted("one giant set of all unique words")
